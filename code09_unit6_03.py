...
def regr_test(features, label):
    ...
    for regr_name, regr in models:
        ...
        for i in range(len(xy_lst)):
            ...
            print(data_lst[i])
            print(regr_name, "conf:", regr.coef_)
            print(regr_name, "MSE", mean_squared_error(Y_part, Y_pred))
            print(regr_name, "MAE", mean_absolute_error(Y_pred, Y_part))
            print(regr_name, "R2", r2_score(Y_part, Y_pred))


def main():
    features, label = hr_preprocessing(sl=True)
    # hr_modeling(features, label)      #分类实验的函数
    # 这三个特征之间有一定相关性，做回归实验
